...
pub unsafe trait Trace<'r> {
    fn trace(t: &Self);
    const TRACE_FIELD_COUNT: u8;
    const TRACE_TYPE_INFO: GcTypeInfo<'r>;
    fn trace_child_type_info() -> Vec<GcTypeInfo<'r>>;
    fn trace_transitive_type_info() -> HashSet<GcTypeInfo<'r>>;
}

#[derive(Debug, Copy, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct GcTypeInfo<'r> {
    trace_ptr: usize,
    drop_ptr: usize,
    needs_drop: bool,
    byte_size: u16,
...
impl<'r, T: Trace<'r>> Deref for Gc<'r, T> {
    type Target = T;
    fn deref(&self) -> &T {
        self.ptr
    }
...
thread_local! {
    ...
    static GC_BUS: HashMap<GcTypeInfo<'static>, Box<Mutex<BusMsg>>> = HashMap::new();
}
...
impl<'r, T: Trace<'r>> Arena<T> {
    ...
    pub fn gc_alloc(&'r self, t: T) -> Gc<'r, T> {
        unimplemented!()
    }
...
unsafe impl<'r, T: NoGc + Immutable> Trace<'r> for T {
    fn trace(_: &T) {}
...
fn list_test() {
    ...
    unsafe impl<'r, T> Trace<'r> for List<'r, T> {
        fn trace(_: &List<'r, T>) {}
        const TRACE_FIELD_COUNT: u8 = 0;
        const TRACE_TYPE_INFO: GcTypeInfo<'r> = GcTypeInfo {
            trace_ptr: todo!(),
            drop_ptr: drop_in_place as *const fn(*mut Self) as usize,
            needs_drop: needs_drop::<Self>(),
            byte_size: size_of::<T>() as u16,
            alignment: align_of::<T>() as u16,
            r: PhantomData,
        };
